...
def get_solution(model, D, TENSOR, COURIERS, NODES):
    all_paths = []
...
def main(args):
    if args.verbose:
        print(f"Running instance {args.instance} with {args.model} model")
    ...
    # 2) Each node is entered and left just once by any vehicle
    ...
    model = s.model()
    all_paths = get_solution(model, D, TENSOR, COURIERS, NODES)
    write_json_solution(args.instance,  'SMT', f'z3 {args.model}', (time.time() - start).__floor__(), False, model[obj].as_long(), all_paths)
    ...
    while satisfiable:
        elapsed_time = (time.time() - start).__floor__()
        ...
        # If the status is satisfiable, set the upper bound to the objective value
            ...
            all_paths = get_solution(model, D, TENSOR, COURIERS, NODES)
            write_json_solution(args.instance,  'SMT', f'z3 {args.model}', elapsed_time, optimality, model[obj].as_long(), all_paths)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--timeout', type=int, metavar='--t', help='Timeout for the solver', default=290, required=False)
    ...
    if args.runall:
        ...
        for instance in instances:
            ...
            p = Process(target=main, args=(instance_args,), name=f'Process_{instance}') # TODO: fix saving of the results before timeout of process
            p.start()
            p.join(timeout=args.timeout)
            p.terminate()

    elif args.instance:
        p = Process(target=main, args=(args,), name=f'Process_{args.instance}')
        p.start()
        p.join(timeout=args.timeout)
        p.terminate()
    else:
