...
def main(args):
    ...
    if args.model == 'SB':
        # 7) Symmetry breaking constraint: impose order on first node to visit
        for k in range(COURIERS-2):
            for i in range(NODES):
                for j in range(i + 1, NODES):
                    cvrp_model += plp.LpConstraint(TENSOR[0][j][k] + TENSOR[0][i][k+1], sense=plp.LpConstraintLE, rhs=1, name=f"Symmetry_breaking_{k}_{i}_{j}")
